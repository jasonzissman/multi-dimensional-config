  ...
  return (
    ...
    !!matches.find((p1) =>
      Object.keys(parameters).find(
        (key) =>
          p1.key === key &&
          satisfiesCondition(p1, parameters[key], p1.comparator || 'eq')
      )
    )
  );
};

const lookUpConfigValueRecursive = (
  config: ConfigNode,
  parameters: MatchingParameter,
  depth: number
): any => {
  ...
  if (atLeastOneParamMatches(parameters, config.matches)) {
    const matchedSubgroup = config.subGroups?.find((g) =>
      atLeastOneParamMatches(parameters, g.matches)
    );
    return matchedSubgroup
      ? lookUpConfigValueRecursive(matchedSubgroup, parameters, depth + 1)
      : config.resolvedValue;
  }
};

export const lookUpConfigValue = (
  config: ConfigNode,
  parameters: MatchingParameter
): any => {
